...
int main(int argc, char *argv[]){	//argc is the number of arguments; argv is the argument
	...
	string locTempString, locTempName, locName="tree_", loc_filenum;
	...
	for(loc_i = 1; loc_i < argc; loc_i++){
		if(argv[loc_i][0] != '-'){
			...
			if (loc_times == 0){
				...
				locTempName=locTempName.substr(7,locTempName.length()-11);//get the run number and file extension
				locName.append(locTempName);
				locName.append(".root");
				loc_filenum=locTempName;
				loc_times++;
			}
			numbbos++;
		}
    ...
    int loc_numofpart, loc_num_pos, loc_num_neg, loc_num_neu, loc_head_eventnum, loc_head_runnum, loc_head_time, loc_head_filenum;
    ...
    float loc_inv_mass_01, loc_inv_mass_02, loc_inv_mass_12, loc_beam_x, loc_beam_y, loc_beam_z;
    ...
    vector<int> loc_mctk_label;
    vector<int> loc_mctk_id;
    vector<TLorentzVector> loc_mctk_track;
    
    locTree->Branch("mctk_track",&loc_mctk_track);
    locTree->Branch("mctk_label", &loc_mctk_label);
    locTree->Branch("mctk_id", &loc_mctk_id);
    ...
    locTree->Branch("head_filenum", &loc_head_filenum, "head_filenum/I");
    ...
    int num_evnt_row, num_tagr_row, num_mctk_row, st_stat, dc_stat, sc_stat, tagrstat, temp_fid_sec;
    ...
    for (loc_numbbos=0; loc_numbbos<numbbos; loc_numbbos++){
        ...
        while(getBOS(&bcs_,1,"E")!=0){
            ...
            locNumEvents++;
            if((Get_dbEVNTbank() != NULL)&&(Get_dbSTPBbank() != NULL)&&(Get_dbTAGRbank() != NULL)&&(Get_dbSCPBbank() != NULL) && (Get_dbDCPBbank() != NULL) && (Get_dbMCTKbank() != NULL)){// if bank exists
                //cout<<locNumEvents<<endl;

                num_evnt_row = Get_dbEVNTbank()->bank.nrow;//get number of tracks in EVNT bank for that event
                num_tagr_row = Get_dbTAGRbank()->bank.nrow;//get number of tracks in TAGR bank for that event
                num_mctk_row = Get_dbMCTKbank()->bank.nrow;//get number of tracks in MCTK bank for that event
                
                loc_mctk_label.clear();
                loc_mctk_track.clear();
                loc_mctk_id.clear();
                loc_numofpart=0;
                loc_num_good_ph=0;
                loc_num_pos=0;
                ...
                if (loc_numofpart >= 3 && loc_num_pos==2 && loc_num_neg==1){
                    ...
                    loc_head_filenum = atoi(loc_filenum.c_str());
                    ...
                    for (int loc_xxmm=0; loc_xxmm<num_mctk_row; loc_xxmm++){
                        mctkpm=Get_dbMCTKbank()->mctk[loc_xxmm].pmom;
                        mctkcx=Get_dbMCTKbank()->mctk[loc_xxmm].cx;
                        mctkcy=Get_dbMCTKbank()->mctk[loc_xxmm].cy;
                        mctkcz=Get_dbMCTKbank()->mctk[loc_xxmm].cz;
                        mctkmass=Get_dbMCTKbank()->mctk[loc_xxmm].mass;
                        tlorentztemp.SetXYZM(mctkpm*mctkcx,mctkpm*mctkcy,mctkpm*mctkcz,mctkmass);
                        loc_mctk_track.push_back(tlorentztemp);
                        loc_mctk_label.push_back(Get_dbMCTKbank()->mctk[loc_xxmm].flag);
                        loc_mctk_id.push_back(Get_dbMCTKbank()->mctk[loc_xxmm].id);
                    }

                    locTree->Fill();
                    /*if ((loc_delt_be[0]>-0.1&&loc_delt_be[0]<0.1)&&(loc_delt_be[1]>-0.1&&loc_delt_be[1]<0.1)&&(loc_delt_be[2]>-0.2&&loc_delt_be[2]<0.2)&&(loc_inv_mass_02<1.2||loc_inv_mass_12<1.2)&&(loc_mctk_label[4]>0)){
                        locTree->Fill();
