void transferSigma(){
  ...
  ifstream input("Sigma");

  ofstream output("SigmaKS");
  ...
  while(!input.eof()){
    ...
    if(w>1.77+wIncreaseIndex*0.04-0.01 && w<1.77+wIncreaseIndex*0.04+0.01){
      vectw.push_back(w);
	...
	for(int j=0;j<vectcostheta.size();j++){
	  if(vectcostheta[j]>costhetacalc-0.05 && vectcostheta[j]<costhetacalc+0.05){
	    output<<setw(15)<<setiosflags(ios::left)<<w-0.04<<setw(15)<<setiosflags(ios::left)<<costhetacalc<<setw(15)<<setiosflags(ios::left)<<vectobs[j]<<endl;
	    flag=1;
	    break;
	  }
	}
	if(flag==0){
	  if(costhetacalc<vectcostheta[0]){
	    output<<setw(15)<<setiosflags(ios::left)<<w-0.04<<setw(15)<<setiosflags(ios::left)<<costhetacalc<<setw(15)<<setiosflags(ios::left)<<vectobs[0]<<endl;
	  }
	  else if(costhetacalc>vectcostheta[vectcostheta.size()-1]){
	    output<<setw(15)<<setiosflags(ios::left)<<w-0.04<<setw(15)<<setiosflags(ios::left)<<costhetacalc<<setw(15)<<setiosflags(ios::left)<<vectobs[vectcostheta.size()-1]<<endl;
	  }
	  else{
	    min=2;
	    for(int j=0;j<vectcostheta.size();j++){
	    ...
	    if(fabs(vectcostheta[jindex]-costhetacalc)<=0.100000000001){
	      ...
	      output<<setw(15)<<setiosflags(ios::left)<<w-0.04<<setw(15)<<setiosflags(ios::left)<<costhetacalc<<setw(15)<<setiosflags(ios::left)<<obscalc<<endl;
	    }
	    else{
	      obscalc=(vectobs[jindex-1]-vectobs[jindex])/(vectcostheta[jindex-1]-vectcostheta[jindex])*(costhetacalc-vectcostheta[jindex])+vectobs[jindex];
	      output<<setw(15)<<setiosflags(ios::left)<<w-0.04<<setw(15)<<setiosflags(ios::left)<<costhetacalc<<setw(15)<<setiosflags(ios::left)<<obscalc<<endl;
